...
- (NSArray *)titleAry
{
    if (!_titleAry) {
        ...
        _titleAry = @[@"1.简单工厂模式", @"2.策略模式", @"3.装饰模式", @"4.代理模式", @"5.工厂方法模式", @"6.原型模式", @"7.模板模式", @"8.外观模式", @"9.建造者模式", @"10.观察者模式", @"11.抽象工厂模式", @"12.状态模式", @"13.适配器模式", @"14.备忘录模式", @"15.组合模式", @"16.迭代器模式", @"17.单例模式", @"18.桥接模式"];
    }
